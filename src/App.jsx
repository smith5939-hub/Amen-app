...
function SignIn({ onSignIn }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.cream, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 32 }}>
      ...
      <input style={{ ...inputStyle, maxWidth: 280, textAlign: "center", marginBottom: 16 }} placeholder="Your first name..." value={name} onChange={e => setName(e.target.value)} />
      <button onClick={() => { if (name.trim()) onSignIn(name.trim()); }} style={{ display: "flex", alignItems: "center", gap: 12, background: T.white, border: `1.5px solid ${T.parchment}`, borderRadius: 14, padding: "14px 24px", cursor: "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", fontFamily: "'DM Sans', sans-serif", fontSize: 15, fontWeight: 500, color: T.ink, opacity: name.trim() ? 1 : 0.5 }}>
